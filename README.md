...
| `/joke_add <text>` | Add a joke to the daily joke rotation. The joke pool is global (shared across every server). |
| `/joke_activation <time>` | Activate the daily joke in the **current channel and server** at a given time (e.g. `14:00`). Each server is configured independently — running this in two servers sets up two independent daily-joke schedules. |
| `/joke_deactivation` | Stop the daily joke in the current server. The server's sent-joke history is preserved, so re-activating later doesn't replay jokes it already received. |
| `/joke_status` | Show the current server's daily-joke config (channel, time, last sent date) or "not activated". Reply is ephemeral. |
...
| `api.py` | Flask REST API for managing responses, reminders, the joke pool, per-guild daily-joke schedules (`/jokes/guilds`, `/jokes/guilds/<guild_id>`), and wishlist items externally. `POST /wishlist/add` does a live scrape during the request so unscrapeable URLs are rejected immediately instead of becoming dead rows. |
...
| `jokes.py` | `JokesFeature` | The `/joke_add`, `/joke_activation`, `/joke_deactivation`, and `/joke_status` commands plus a 30-second loop that posts one unsent joke per **subscribed guild** per day at each guild's configured time and recycles the pool when exhausted. Sent-history is tracked per guild (`guild_joke_sent` table) so the same joke can run in different servers independently while preserving the no-repeats contract within each server. On first start after the multi-guild upgrade, `_migrate_legacy_config` auto-imports the old single-guild config by looking up the legacy channel via `client.fetch_channel` and writing a `guild_joke_config` row. |
