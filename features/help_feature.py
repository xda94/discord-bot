...
HELP_TEXT = (
    ...
    "Refresh one tracked item immediately and show its source, current status, "
    "and target progress. One refresh per item every five minutes.\n\n"
    ...
    "**/flight_tracker_add** `<origin>` `<destination>` `<start_date>` `<end_date>` "
    "`[adults]` `[currency]`\n"
    ...
    "**/llm_inactivity** `<activate|deactivate>`\n"
    ...
    "in `.env`. One request per user at a time with a cooldown after completion. "
