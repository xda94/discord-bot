...
HELP_TEXT = (
    ...
    "notice. Deactivation stops capture/use but retains compact profiles.\n\n"
    "**/llm-memory-purge** `<confirmation>`\n"
    "Server managers can enter `PURGE` to permanently remove every saved profile "
    "for this server. Channel settings and individual opt-outs remain unchanged.\n\n"
    "**/memory-show**\n"
    "Privately show your compact saved profile for this server or DM.\n\n"
    "**/memory-forget**\n"
    "Erase your saved profile and pending observations here without opting out.\n\n"
    "**/memory-opt-out** / **/memory-opt-in**\n"
    "Stop and erase persistent memory for yourself, or enable it again. DM memory "
    "is separate from server profiles and requires explicit opt-in.\n\n"
    "**@bot** (mention)\n"
    ...
    "in `.env`. In memory-enabled channels, the same requester's compact server "
    "profile supplements recent chat history. One request per user at a time with "
    "a cooldown after completion. "
    "Set `BOT_ID` in `.env` to your bot's user ID.\n\n"
    "Rate an LLM mention reply with 👍 or 👎. Only the person who asked can submit "
    "that compact feedback signal.\n\n"
