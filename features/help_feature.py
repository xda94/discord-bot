...
HELP_TEXT = (
    ...
    "through Ollama to rewrite it in that mood while keeping the same gist. "
    ...
    "**/flight-tracker add** `<origin>` `<destination>` `<start_date>` `<end_date>` "
    "`[trip_days]` `[adults]` `[currency]`\n"
    "Track round-trip prices per user using 3-letter IATA codes and YYYY-MM-DD dates. "
    "Without `trip_days`, start/end are exact departure/return dates. With (for example) "
    "`trip_days:10`, the period is searched as inclusive sliding windows: day 1-10, "
    "day 2-11, and so on. The bot checks every 5 hours by default and DMs when it finds "
    "the first price or a lower price.\n\n"
    "**/flight-tracker show**\n"
    "List your saved routes, periods, tracker IDs, and latest prices.\n\n"
    "**/flight-tracker delete** `<tracker_id>`\n"
    ...
    "and unloaded after answering (`keep_alive: 0`). Your question is posted right away; "
    ...
    "reply; ping with text gets an LLM answer to that text. Model: `MENTION_OLLAMA_MODEL` "
    "in `.env`. Shares cooldown/queue with `/ask` but stays silent until the reply. "
